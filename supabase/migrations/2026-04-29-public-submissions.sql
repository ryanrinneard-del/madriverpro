...
drop policy if exists "public_submissions_coach_select" on public_submissions;
create policy "public_submissions_coach_select" on public_submissions for select
  using (is_coach(auth.uid()));

drop policy if exists "public_submissions_coach_delete" on public_submissions;
...
-- (replace with a fresh submission once the API is deployed).
-- =====================================================================
insert into public_submissions (full_name, email, submission_data, created_at)
values (
  'Reid',
  'reid@placeholder.invalid',  -- Ryan: replace with Reid's real email from your inbox
  jsonb_build_object(
    'S1 Age',          '16',
    'S1 Years',        '7',
  ...
  now() - interval '1 day'  -- date it yesterday so it sits naturally in the list
);
